...
def render_wipe_layer(layer, t):
    ...
    mask = Image.new("L", VIDEO_SIZE, 0); draw = ImageDraw.Draw(mask)
...
def baca_semua_berita(file_path):
    ...
    except FileNotFoundError: print(f"❌ File '{file_path}' tidak ditemukan!"); exit(1)
    except Exception as e: print(f"❌ Error reading file '{file_path}': {e}"); exit(1)

    blok_berita = content.strip().split("---"); semua_data = []
